...
public class OCECore {
	private List<Edge> edges;
	private List<Node> vertices;
	private static OCECore core;

	//Making it singleton
	private OCECore() {}

	public static OCECore getInstance() {
		if(core == null)
			core = new OCECore();
	...
	public IGraphResponse sendIGraphRequest () {
		Edge [] edgeArray = edges.toArray(new Edge[0]);
		Node [] nodeArray = vertices.toArray(new Node[0]);
		int count = graphSize(nodeArray, edgeArray);
	...
	public static void main(String [] args) {
		OCECore client = OCECore.getInstance();
		client.init();
		IGraphResponse response = client.sendIGraphRequest();
		System.out.println(response);
	}
	
	
	static {
        System.loadLibrary("OCECore");
    }
